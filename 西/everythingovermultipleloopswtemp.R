...
ggplot() +
  ...
  # Temperature (subtle dashed gray)
  geom_line(
    data = output_df,
...
ggplot(birth_df, aes(x = Temperature, y = BirthRate)) +
  geom_line(linewidth = 1.4, color = "sienna2") +
  theme_classic() +
  labs(
    title = "Mosquito Birth Rate vs Temperature",
    x = "Temperature (°C)",
...
#multiple runs w/ plot 2
runs <- 10
temp.range <- seq(18, 34, by = 0.25)
mean.transmission <- numeric(length(temp.range))
mean.inf.mosq <- numeric(length(temp.range))
mean.sus.mosq <- numeric(length(temp.range))
Temp.runs <- matrix(NA, nrow = length(times), ncol = runs)
for(i in 1:runs){
  Temp.runs[,i] <- generate_temp_noise(times)
}
Temp.mean <- rowMeans(Temp.runs)
Temp.sd <- apply(Temp.runs, 1, sd)
for(i in seq_along(temp.range)){
  Dummy.Annual.Mean.Temp <- temp.range[i]
  RunVsHistoricalAnnualMeanTemp <- Dummy.Annual.Mean.Temp / Historical.Temp.AnnualMean
  DayTemp <- DayTemp01 * RunVsHistoricalAnnualMeanTemp
  TempSpline <- smooth.spline(times, DayTemp, spar = 0.6)
  run.results <- numeric(runs)
  for(j in 1:runs){
    Temp.w.Noise <- generate_temp_noise(times)
    TempFun <- approxfun(times, Temp.w.Noise, rule = 2)
    output <- ode(
    ...
    output_df <- as.data.frame(output)
    run.results[j] <- mean(output_df$Inf.Humans / Total.Humans)
    mean_inf_m_run <- mean(output_df$Inf.Mosquitoes)
    mean_sus_m_run <- mean(output_df$Sus.Mosquitoes)
    if(j == 1){
      inf_store <- numeric(runs)
      sus_store <- numeric(runs)
    }
    inf_store[j] <- mean_inf_m_run
    sus_store[j] <- mean_sus_m_run
}
  mean.transmission[i] <- mean(run.results)
  mean.inf.mosq[i] <- mean(inf_store)
  mean.sus.mosq[i] <- mean(sus_store)
}

transmission_df <- data.frame(
  MeanTemp = temp.range,
  MeanAnnualPrevalence = mean.transmission * 4000
)

mosq_df <- data.frame(
  MeanTemp = temp.range,
  MeanInfMosquitoes = mean.inf.mosq,
  MeanSusMosquitoes = mean.sus.mosq
)
...
mosq_long <- pivot_longer(
  mosq_df,
  cols = c("MeanInfMosquitoes", "MeanInfMosquitoes"),
  names_to = "Type",
  values_to = "Population"
)

ggplot(mosq_long, aes(x = MeanTemp, y = Population, color = Type)) +
  geom_line(linewidth = 1.4) +
  scale_color_manual(
    values = c(
      "MeanInfMosquitoes" = "red3"
    ),
  ) +
  theme_classic() +
  labs(
    title = "Mosquito Populations vs Average Annual Temperature",
    x = "Mean Annual Temperature (°C)",
    y = "Mean Mosquito Population"
  )


output_df$Inf.Mosquitoes[355]/(output_df$Sus.Mosquitoes[355]+output_df$Inf.Mosquitoes[355])