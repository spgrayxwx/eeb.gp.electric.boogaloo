# added stochasticity to rain + multiple runs for rain graphed as well w/ temp
library(deSolve)
library(ggplot2)
library(tidyr)

#set temp + rainfall variability
Temp_Variability <- 1.5
Rain_Variability <- 0.25
set.seed(11037)
...
generate_temp_noise <- function(times, variability = Temp_Variability, phi = 0.7) {
...
#rain smoothing + stochasticity
RainfallSpline <- smooth.spline(times, Rainfall01, spar = 0.6)
rain_smoothed <- predict(RainfallSpline, times)$y
generate_rain_noise <- function(times, variability = Rain_Variability, phi = 0.7) {
  base_rain <- predict(RainfallSpline, times)$y
  n <- length(times)
  noise <- numeric(n)
  noise[1] <- rnorm(1, 0, variability)
  for(i in 2:n){
  ...
  noise <- noise - mean(noise)
  rain <- base_rain * (1 + noise)
  rain[rain < 0] <- 0
  return(rain)
}
Rain.w.Noise <- generate_rain_noise(times)
RainFun <- approxfun(times, Rain.w.Noise, rule = 2)
Rain_min <- min(Rain.w.Noise)
Rain_max <- max(Rain.w.Noise)
...
#multiple runs
temp.range <- seq(18, 34, by = 0.25)
mean.transmission <- numeric(length(temp.range))
for(i in seq_along(temp.range)){
  Dummy.Annual.Mean.Temp <- temp.range[i]
  RunVsHistoricalAnnualMeanTemp <- Dummy.Annual.Mean.Temp / Historical.Temp.AnnualMean
  DayTemp <- DayTemp01 * RunVsHistoricalAnnualMeanTemp
  TempSpline <- smooth.spline(times, DayTemp, spar = 0.6)
  Temp.w.Noise <- generate_temp_noise(times)
  ...
  output_df <- as.data.frame(output)
  mean.transmission[i] <- mean(output_df$Inf.Humans / Total.Humans)
}

transmission_df <- data.frame(
  MeanTemp = temp.range,
  MeanAnnualPrevalence = mean.transmission * 400
  #first 4 for 4 seasons, 100 to scale it up to be per thousand individuals
)

ggplot(transmission_df, aes(x = MeanTemp, y = MeanAnnualPrevalence)) +
  geom_line(linewidth = 1.4, color = "cadetblue2") +
  theme_classic() +
  labs(
    title = "Malaria Prevalence vs Average Annual Temperature",
    x = "Mean Annual Temperature (°C)",
    y = "Mean Malaria Prevalence per Thousand Individuals"
  )

#adding this but 4 rain
rain.mean.range <- seq(1, 12, by = 0.25)
mean.transmission.4.rain <- numeric(length(rain.mean.range))

for(i in seq_along(rain.mean.range)){
  Dummy.Annual.Mean.Rain <- rain.mean.range[i]
  Historical.Rain.AnnualMean <- mean(Rainfall01)
  RunVsHistoricalRainMean <- Dummy.Annual.Mean.Rain / Historical.Rain.AnnualMean
  Rainfall_scaled <- Rainfall01 * RunVsHistoricalRainMean
  RainfallSpline <- smooth.spline(times, Rainfall_scaled, spar = 0.6)
  rain_smoothed <- predict(RainfallSpline, times)$y
  noise <- numeric(length(times))
  noise[1] <- rnorm(1,0,Rain_Variability)
  for(j in 2:length(times)){
    noise[j] <- 0.7 * noise[j-1] + rnorm(1,0,Rain_Variability)
  }
  noise <- noise - mean(noise)
  Rain.w.Noise <- rain_smoothed * (1 + noise)
  Rain.w.Noise[Rain.w.Noise < 0] <- 0
  RainFun <- approxfun(times, Rain.w.Noise, rule = 2)
  Rain_min <- min(Rain.w.Noise)
  Rain_max <- max(Rain.w.Noise)
  output <- ode(
  ...
  output_df <- as.data.frame(output)
  mean.transmission.4.rain[i] <- mean(output_df$Inf.Humans / Total.Humans)
}

rain_transmission_df <- data.frame(
  MeanRainfall = rain.mean.range,
  MeanAnnualPrevalence = mean.transmission.4.rain * 400
)

ggplot(rain_transmission_df, aes(x = MeanRainfall, y = MeanAnnualPrevalence)) +
  geom_line(linewidth = 1.4, color = "plum") +
  theme_classic() +
  labs(
    title = "Malaria Prevalence vs Mean Annual Rainfall",
    x = "Mean Rainfall (mm/day)",
    y = "Mean Malaria Prevalence per Thousand Individuals"
  )