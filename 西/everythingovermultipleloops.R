...
for(i in seq_along(temp.range)){
  ...
  output_df <- as.data.frame(output)
  mean.transmission[i] <- mean(output_df$Inf.Humans)
}

transmission_df <- data.frame(
  MeanTemp = temp.range,
  MeanAnnualPrevalence = mean.transmission * (4 * 100 * 5)
  #first 4 for 4 seasons, 100 to scale it up to be per thousand individuals, 5 to make it
  #work
)
...
ggplot(transmission_df, aes(x = MeanTemp, y = MeanAnnualPrevalence)) +
  ...
  theme_classic() +
  labs(
    title = "Malaria Prevalence vs Annual Temperature",
